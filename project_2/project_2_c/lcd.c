...
// Function to track the position of the cursor on the LCD
unsigned char LCD_position(unsigned char position){
	
	unsigned char changed_pos;
	
	if (position == (0x80 | 0x07)){
		changed_pos = (0x80 | 0x40);
	}else if (position == (0x80 | 0x47)){
		changed_pos = (0x80 | 0x00);
	}else{
		changed_pos = position + 0x01;
	}
